...
instance Monad f => Applicative (StateT s f) where
  ...
  StateT rstfAB <*> StateT rstfA = StateT { runStateT = rst }
    -- where rst s = rstfAB s >>= \(ab, t) -> ff ab <$> rstfA t
...
instance Monad f => Monad (StateT s f) where
  ...
  a2stfb =<< StateT rstfA = StateT { runStateT = rs }
    ...
    -- where rs s = (\(a, t) -> runStateT (a2stfb a) t) =<< rstfA s
    -- where rs = (\(a, t) -> runStateT (a2stfb a) t) <=< rstfA
...
runState' :: State' s a
          -> s
          -> (a, s)
-- runState' (StateT rs) s = runExactlyOne (rs s)
...
-- Empty
--
distinctF :: forall a. (Ord a, Num a)
          => List a
          -> Optional (List a)
...
instance Monad f => Applicative (OptionalT f) where
  ...
  -- TODO: /Tip:/ Use `onFull` to help implement (<*>).
  -- onFull :: (a -> f (Optional b)) -> Optional a -> f (Optional b)
  -- (<*>)  :: OptionalT f (a -> b) -> OptionalT f a -> OptionalT f b
  --
  (<*>) :: forall a b. OptionalT f (a -> b)
        -> OptionalT f a
        -> OptionalT f b
  OptionalT ab <*> OptionalT a = OptionalT { runOptionalT = rot }
    where rot :: f (Optional b)
          -- rot = pure z <*> ab <*> a
          -- rot = z <$> ab <*> a
          -- z :: Optional (a -> b) -> Optional a -> Optional b
          -- z Empty    _        = Empty
          -- z _        Empty    = Empty
