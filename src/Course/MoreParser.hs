...
-- string Nil     = pure Nil
-- string (c:.cs) = (:.) <$> is c <*> string cs

-- using foldRight
-- string = foldRight (\a b -> (:.) <$> is a <*> b) (pure Nil)
...
betweenCharTok :: Char
               ...
               -> Parser a
               -> Parser a
-- betweenCharTok o c pa = between (is o) (is c) pa
...
--
hex :: Parser Char
hex = chr . hxInt <$> replicateA 4 (satisfy isHexDigit)
  where hxInt c = case readHex c of
                    Empty  -> 0
                    Full n -> n
...
sepby1 :: Parser a
       -> Parser s
       -> Parser (List a)
-- sepby1 pa ps = pa >>= (\x -> list (ps >> pa) >>= (\xs -> pure $ x :. xs))
sepby1 pa ps = do
  x <- pa
  xs <- list (ps >> pa)
  pure $ x :. xs
...
-- True
--
betweenSepbyComma :: Char
                  -> Char
                  -> Parser a
                  -> Parser (List a)
-- betweenSepbyComma o c pa = betweenCharTok o c $ sepby pa (charTok ',')
betweenSepbyComma o c pa = betweenCharTok o c $ pa `sepby` charTok ','
